...
// Spawn dazzle particles only on front page
let dazzleInterval = setInterval(spawnDazzle, 180);
...
(function() {
  ...
  const stars = Array.from({length:220}, () => ({
    ...
    r:Math.random()*1.3+0.2, a:Math.random(), da:(Math.random()-.5)*.007
  }));
  (function draw() {
    ...
    stars.forEach(s => {
      ...
      sx.fillStyle=`rgba(210,220,255,${s.a})`; sx.fill();
    });
...
function resizeCanvas() {
  const s=Math.min(innerWidth/1000,(innerHeight-52)/600,2);
  canvas.style.width=(1000*s)+'px'; canvas.style.height=(600*s)+'px';
}
resizeCanvas(); window.addEventListener('resize',()=>{ resizeCanvas(); if(inGame) renderGame(); });
...
function setOverlay(id) {
  ...
  wrap.classList.toggle('active',!id);
  const isGame=!id&&inGame;
  document.getElementById('hud').style.display=id?'none':'';
  document.getElementById('player-panel').style.display=isGame?'':'none';
  document.getElementById('chat-panel').style.display=isGame?'':'none';
  if(!id&&inGame) stopDazzle();
}
function closeModal() {
  if(inGame){
...
document.getElementById('guest-btn').addEventListener('click',()=>{
  playSound('click'); myName='Guest'+Math.floor(1000+Math.random()*9000); myToken=null; myPfp=null; enterLobby();
});
// Auto-login
if(myToken&&myName){ document.getElementById('auth-overlay').style.transition='none'; enterLobby(); }

function enterLobby() {
  inGame=false; setOverlay('lobby-overlay');
  document.getElementById('lobby-my-name').textContent=myName;
  document.getElementById('lobby-my-pfp').innerHTML=myPfp?`<img src="${myPfp}">`:'🎮';
  if(!socket.connected) socket.connect();
  else socket.emit('requestRoom',{playerName:myName,token:myToken,pfp:myPfp});
}
...
function doRejoin() {
  ...
  leaveVC();
  // Restart dazzle for lobby
  dazzleInterval=setInterval(spawnDazzle,180);
  setOverlay('lobby-overlay');
  socket.emit('rejoinLobby');
}
document.getElementById('play-again-btn').addEventListener('click',doRejoin);
...
socket.on('assignedRoom',name=>{
  room=name;
  document.getElementById('lobby-room-name').textContent=name;
...
socket.on('unoAlert',({id,name})=>{
  ...
  updatePlayerPanel(); setTimeout(()=>{unoPlayers.delete(id);updatePlayerPanel();},8000);
});
...
socket.on('playerDisconnect',()=>{ showNotify('A player disconnected','error'); setTimeout(doRejoin,2500); });
...
canvas.addEventListener('touchend',onInteract,{passive:false});
canvas.addEventListener('mousemove',e=>{
  const{X,Y}=getCanvasCoords(e),prev=hoveredCard;
  hoveredCard=getCardAtPos(X,Y); if(hoveredCard!==prev) renderHand();
});
canvas.addEventListener('mouseleave',()=>{hoveredCard=-1;renderHand();});
...
document.querySelectorAll('.color-btn').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  if(pendingWild!==null){playSound('play');socket.emit('playCard',[pendingWild,room,color]);pendingWild=null;}
}));
...
function renderHand(){
  ctx.clearRect(0,390,1000,210);
  const layout=getCardLayout(); if(!layout.length)return;
  ctx.save(); ctx.fillStyle='rgba(5,5,25,.32)'; rrect(10,396,980,198,13); ctx.fill();
  ctx.strokeStyle='rgba(140,160,255,.1)'; ctx.lineWidth=1; ctx.stroke(); ctx.restore();
  layout.forEach(({x,y,w,h},i)=>{
    const cn=hand[i],isHov=hoveredCard===i,isPlay=canPlay(cn),liftY=isHov?y-22:y;
    ctx.save();
    if(isTurn&&isPlay){ctx.shadowBlur=isHov?46:20;ctx.shadowColor=glowCol(cn);}
    else ctx.globalAlpha=.62;
    ctx.drawImage(cards,1+cdW*(cn%14),1+cdH*Math.floor(cn/14),cdW,cdH,x,liftY,w,h);
    if(isTurn&&isPlay&&!isHov){ctx.globalAlpha=.1;ctx.fillStyle='#fff';ctx.fillRect(x,liftY,w,h);}
    ctx.restore();
  });
  if(isTurn){ctx.fillStyle='rgba(0,217,255,.55)';ctx.font='600 10px "Exo 2"';ctx.textAlign='center';ctx.fillText('↑ YOUR CARDS — TAP TO PLAY',500,402);}
}
